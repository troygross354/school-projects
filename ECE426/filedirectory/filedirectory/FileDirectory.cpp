...
bool FileDirectory::dlete(char filename[])
{
	...
	unsigned int nextClusterAddress;
	...
	for(int i = firstClusterAddress; FAT16[i] <= 0xFFF8; i++)
	{
		FAT16[i] = 0x0000;
	}
	return true;
...
bool FileDirectory::write(char filename[], int numberBytes, char inputData[], int year, int month, int day, int hour, int minute, int second)
{
	...
	unsigned int i, firstClusterAddress, nextClusterAddress;
	...
	while(allocatedBytes < numberBytes)
	{
		...
		for (i = firstClusterAddress + 1; i < 256; i++)
		{
			if(FAT16[i] == 0 || FAT16[i] == 1)
			{
				FAT16[i] == firstClusterAddress;
				firstClusterAddress = i;
				allocatedBytes += 4;
				break;
			}
			if(i == 255)
			{
				//Not enough free clusters to store file
				return false;
			}
		}
	}
	//Set the end of the file marked address
	FAT16[i] = 0xFFFF;
	...
	for (int i = 0; i <= numClusters; i++)
	{
		for (int j = 0; j < 4; j++)//Each cluster has 4 bytes
		{
			data[FAT16[firstClusterAddress] << 2) + j] = inputdata[p]; // data[cluster * 4bytes + offset inside of cluster
			++p;//File data index index increment
		}
		firstClusterAddress = FAT16[firstClusterAddress];
	}
	return true;
