...
bool FileDirectory::create(char filename[], unsigned short int numberBytes)
{
	...
	for (i = 0; i < 256 && sCLIndex < numClusters; i++)
	{
		if (FAT16[i] == 0 || FAT16[i] == 1)
		{
			//Store next available FAT address
			storeClusterLocations[sCLIndex++] = i;
		}
		else if(i == 255) {
...
bool FileDirectory::dlete(char filename[])
{
	...
	unsigned int firstClusterAddress, nextClusterAddress;
	...
	//Clear all clusters for file
	for(int i = firstClusterAddress; FAT16[i] <= 0xFFF8; i = nextClusterAddress)
	{
...
bool FileDirectory::write(char filename[], unsigned int numberBytes, char inputData[], int year, int month, int day, int hour, int minute, int second)
{
	...
	unsigned int i, freeClusterAddress;
	unsigned int allocatedBytes = 0, numClusters = numberBytes / 4;
	
	//TODO: Search for filename

	//TODO: (3)	to write / update the file name; extension; date; time; file length and first cluster address into the first unused entry in the File Directory;*/
	...
	fileDirectory[empty][27] = freeClusterAddress >> 8;
	fileDirectory[empty][26] = freeClusterAddress;
	...
	for (i = 0; i <= numClusters; i++)
	{
		for (int j = 0; j < 4; j++)//Each cluster has 4 bytes
		{
			data[(FAT16[freeClusterAddress] << 2) + j] = inputData[p]; // data[cluster * 4] + offset inside of cluster
			++p;//File data index index increment
		}
...
void FileDirectory::printData(char filename[])
{
	...
	for (int i = firstSectorAddress; FAT16[i] < 0xFFF8; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			cout << data[(FAT16[i] << 2) + j] << " ";
		}
	}
